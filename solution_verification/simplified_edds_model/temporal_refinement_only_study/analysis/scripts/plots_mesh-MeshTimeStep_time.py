...
#     plt.ylim([295,310])
    ...
    plt.tight_layout()
    plt.savefig(f'figs/{figDir}/streeq_convergence_{var[0]}_{label}.png',dpi=300)
    plt.close()
...
def error_plots(data,model_fit_statistics,dxORdt,label,figDir): 
 ...
 #   plt.title(f'{title}: {var} (scaled)')
    plt.savefig(f'figs/{figDir}/streeq_error_{var[0]}_{label}.png',dpi=300)
    plt.close()
    return err 
...
def  scatter_err_dxdy(data,model_fit_statistics,dataM,errM,dataT,errT,label,numCurves,figDir):
    ...
    im= plt.scatter (data.dx,data.dt, norm=colors.LogNorm(vmin=5e-2, vmax=1e-4),c=err,cmap='viridis',s=200,zorder=5)
 ...
 #   plt.title(f'{title}: {var} (scaled)')
    plt.tight_layout()
    if numCurves == 3:
        plt.savefig(f'figs/{figDir}/streeq_scatter_error_{var[0]}_{label}_all.png',dpi=300)
    if numCurves == 1: 
        plt.savefig(f'figs/{figDir}/streeq_scatter_error_{var[0]}_{label}.png',dpi=300)
    plt.close()
    return err
...
#     plt.loglog(h_ref,ref_slope2,'--',label='p=2')
    ...
    plt.grid(True,which='both',linestyle='--',alpha=0.6)
    plt.tight_layout()
    plt.savefig(f'figs/{figDir}/relative_err_observedP_{label}.png',dpi=300,bbox_inches='tight')    
####################################

#### Functions 
def data_processTime(directory,data_input_file,data_input_file_names):
#     data=pd.read_csv(directory+'/'+data_input_file,names = data_input_file_names,skiprows=1, sep='\\s+')
    data=pd.read_csv(directory+'/'+data_input_file,names = data_input_file_names,skiprows=1,comment='#', sep='\\s+',header=None)
    model_fit_statistics=pd.read_pickle(directory+'/output/model_fit_statistics.pkl')
    model_fits=pd.read_pickle(directory+'/output/model_fits.pkl')
...
### Convergence Plots Mesh & Timestep
def convergence_plots_dxdtTime(data,model_fit_statistics,model_fits,dx,dt,label,figDir):
    tmp=model_fits.loc[var]
    plt.figure(figsize=(12,8))
    for irow in range(len(tmp)):
        beta0,beta1,beta2,beta12,gamma1,gamma2,obj,pvalue=tmp.iloc[irow]
...
### Convergence Plots 
def convergence_plotsTime(data,model_fit_statistics,model_fits,dxt,dxORdt,label,figDir):
    plt.figure(figsize=(12,8))
    tmp=model_fits.loc[var]
    for irow in range(len(tmp)):
        beta0,beta1,gamma1,obj,pvalue=tmp.iloc[irow]
...
# Error Plots
def error_plotsTime(data,model_fit_statistics,dxORdt,label,figDir,varT):
    plt.figure(figsize=(12,8))
    tmp=model_fit_statistics.loc[(varT,0,'beta0')]
    median=tmp['median']
    err = np.array((data[varT]-median).abs()/median)
    plt.title(f'{label}')
    plt.ylabel('Error')
    plt.grid(True,which='both',linestyle='--',alpha=0.6)
    plt.tight_layout()
...
# Scatter dx vs dy - color = error  
#cmap='coolwarm'
def  scatter_err_dxdyTime(data,model_fit_statistics,dataM,errM,dataT,errT,label,numCurves,figDir):
    plt.figure(figsize=(12,8))
    tmp=model_fit_statistics.loc[(var,0,'beta0')]
    median=tmp['median']
    print(f'data.dx={data.dx} \n data.dt={data.dt}')
    ...
    plt.loglog(data.dx,data.dt, label='Mesh & Time',linestyle='-',color='black',zorder=1)
    im= plt.scatter (data.dx,data.dt, norm=colors.LogNorm(vmin=1e-3, vmax=5e-5),c=err,cmap='viridis',s=200,zorder=5)
    cbar = plt.colorbar(im)
    if numCurves == 3:
        plt.xticks(ticks=[0.22840836],labels=[f'{val:.1e}' for val in [0.22840836]],rotation=45)
        plt.yticks(ticks=[1e-4,1e-3,0.005,1e-2],labels=['$10^{-4}$','$10^{-4}$','5.0e-03','$10^{-2}$'],rotation=0)
 ...
 #   plt.title(f'{title}: {var} (scaled)')
    ...
    return err
# Order of Convergence Plots
def order_pTime(err,h,dxORdt,label,figDir):
    p_observed = (np.log(err[-1])-np.log(err[0]))/(np.log(h[-1])-np.log(h[0]))
#     print(f'p_observed: {p_observed}')
    h_anchor = h[0]
    u_err_anchor = err[0]
...
#     plt.loglog(h_ref,ref_slope2,'--',label='p=2')
    ...
    plt.tight_layout()
    plt.savefig(f'figs/{figDir}/relative_err_observedP_{label}.png',dpi=300,bbox_inches='tight')
def plot_TavgTimestepTime(timestep,Tavg,varLabel,figDir):
    plt.figure(figsize=(12,8))
    ...
    plt.tight_layout()
    plt.savefig(f'figs/{figDir}/timestep_vs_Temp_{varLabel}.png',dpi=300,bbox_inches='tight')
...
dataT, model_fit_statisticsT, model_fitsT = data_processTime(directoryT,data_input_fileT,data_input_file_namesT)
#print(f'model_fit_statisticsT:\n {model_fit_statisticsT}')
dt=np.linspace(0,0.0065,101)
####convergence_plotsTime(dataT,model_fit_statisticsT,model_fitsT,dt,'dt',label='time')
figDir = 'study_timestep'
dxORdt='$\Delta t$'
for varT in var_all:
    err_time = error_plotsTime(dataT,model_fit_statisticsT,'dt',varT,figDir,varT)
    err_time, h_time = np.array(err_time), np.array(dataT['dt'])
    order_pTime(err_time,h_time,dxORdt,varT,figDir)
    plot_TavgTimestepTime(dataT['dt'],dataT[varT],varT,figDir)
    ...
    resultsTime.to_csv(f'results/results_Time_Err_{var}.dat',index=False)
...
err_meshTime = scatter_err_dxdy(dataMT, model_fit_statisticsMT,dataM,err_mesh,dataT,err_time,'meshTime',1,figDir)
err_meshTime_2 = scatter_err_dxdy(dataMT, model_fit_statisticsMT,dataM,err_mesh,dataT,err_time,'meshTime',3,figDir)
...
resultsMeshTimestep.to_csv('results/results_MeshTimestep_Err.dat',index=False)
