# Running Dakota to parallelize OpenFOAM Cases (Multiple-Case Template)

Note 1: The missing files in the template are generated by the Dakota workflow using `template.filename`so that the variables could be insterted by dakota for parallelizing the studies. 
Note 2: This requires the Dakota workflow to run this case dependent on the input file
