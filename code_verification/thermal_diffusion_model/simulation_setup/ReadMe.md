...
## Transient Study Modifications - (Co-refine)
Since the study is co-refined the `system/controlDict` file must be modified along with a corresponding mesh resolution (mesh file and modified sbatch file). The corresponding configuration must be manually setup by copy pasting the mesh file and by setting the deltaT in the `template/system/controlDict`: 
- Mesh 0 &  `deltaT  5e-3;`
- Mesh 1 &  `deltaT  5e-4;` 
- Mesh 2 &  `deltaT  5e-5;`
